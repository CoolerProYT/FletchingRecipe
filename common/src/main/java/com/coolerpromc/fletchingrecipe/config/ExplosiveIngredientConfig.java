...
public class ExplosiveIngredientConfig {
    ...
    private static final File CONFIG_FILE = new File(Services.PLATFORM.getConfigDir().toFile(), "fletchingrecipe-explosive-ingredient.json");
    ...
    public static void load() {
        ...
        for (Map.Entry<String, Float> entry : explosiveIngredientMap.entrySet()){
            try {
                ...
                Constants.LOGGER.error("[Explosive Ingredient Config] Invalid item id defined: {}", entry.getKey());
            }
            catch (ItemNotFoundException e){
                Constants.LOGGER.error("[Explosive Ingredient Config] Defined item not found: {}", entry.getKey());
            }
        }
    }
