...
import com.coolerpromc.fletchingrecipe.CommonClientClass;
...
public class ModJEIPlugin implements IModPlugin {
    ...
    public void registerRecipeCatalysts(IRecipeCatalystRegistration registration) {
        registration.addCraftingStation(FletchingCategory.FLETCHING_TYPE, !Services.PLATFORM.isModLoaded("lolmft") ? new ItemLike[]{Blocks.FLETCHING_TABLE} : MoreFletchingTableStation.get());
        registration.addCraftingStation(ExplosiveCategory.EXPLOSIVE_TYPE, !Services.PLATFORM.isModLoaded("lolmft") ? new ItemLike[]{Blocks.FLETCHING_TABLE} : MoreFletchingTableStation.get());
    }

    @Override
    public void registerRecipes(IRecipeRegistration registration) {
        List<JeiFletchingRecipe> fletchingTableRecipes = new ArrayList<>(CommonClientClass.recipeMap.byType(CommonClass.FLETCHING_RECIPE_TYPE.get()).stream().map(RecipeHolder::value).map(recipe -> {
        ...
        BuiltInRegistries.POTION.asHolderIdMap().forEach(potion -> {
            if (Services.PLATFORM.isModLoaded("arrowplus")) ArrowPlusTippedRecipe.register(potion, fletchingTableRecipes);
    ...
    public void registerItemSubtypes(ISubtypeRegistration registration) {
        registration.registerFromDataComponentTypes(Items.ARROW, CommonClass.EXPLOSIVE.get());
        registration.registerFromDataComponentTypes(Items.SPECTRAL_ARROW, CommonClass.EXPLOSIVE.get());
        registration.registerFromDataComponentTypes(Items.TIPPED_ARROW, CommonClass.EXPLOSIVE.get());
    }
}