        ...
        if (!holder.value().getEffects().isEmpty()){
            BasicDisplay.registryAccess().lookupOrThrow(ModRegistries.ARROW_DATA_KEY).asHolderIdMap().forEach(arrowDataHolder -> {
                ...
                List<EntryIngredient> input = List.of(
                        ...
                        EntryIngredients.of(new ItemStack(ModItems.ARROW_PLUS.builtInRegistryHolder(), ClientBoundConfigSyncPacket.INSTANCE.tippedArrowCraftingAmount(), DataComponentPatch.builder().set(ModDataComponents.ARROW_DATA, arrowDataHolder).build()))
                );
    ...
    public static void registerExplosive(Holder<Item> explosiveIngredient, DisplayRegistry registry){
        BasicDisplay.registryAccess().lookupOrThrow(ModRegistries.ARROW_DATA_KEY).asHolderIdMap().forEach(arrowDataHolder -> {
            ...
            List<EntryIngredient> input = List.of(
                    ...
                    EntryIngredients.of(new ItemStack(ModItems.ARROW_PLUS.builtInRegistryHolder(), ClientBoundConfigSyncPacket.INSTANCE.explosiveArrowCraftingAmount(), DataComponentPatch.builder().set(ModDataComponents.ARROW_DATA, arrowDataHolder).build()))
            );
            List<EntryIngredient> output = List.of(EntryIngredients.of(outputStack));
    ...
    public static void registerExplosiveTipped(Holder<Potion> holder, Holder<Item> explosiveIngredient, DisplayRegistry registry){
        if (!holder.value().getEffects().isEmpty()){
            BasicDisplay.registryAccess().lookupOrThrow(ModRegistries.ARROW_DATA_KEY).asHolderIdMap().forEach(arrowDataHolder -> {
                ...
                List<EntryIngredient> input = List.of(
                        EntryIngredients.of(new ItemStack(explosiveIngredient, 1)),
                        EntryIngredients.of(new ItemStack(ModItems.ARROW_PLUS.builtInRegistryHolder(), ClientBoundConfigSyncPacket.INSTANCE.explosiveArrowCraftingAmount(), DataComponentPatch.builder().set(ModDataComponents.ARROW_DATA, arrowDataHolder).set(DataComponents.POTION_CONTENTS, new PotionContents(holder)).build()))
                );
                List<EntryIngredient> output = List.of(EntryIngredients.of(outputStack));
